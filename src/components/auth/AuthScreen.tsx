...
export const AuthScreen: React.FC = () => {
  ...
  const handleGuestDemo = () => {
    login('alex.morgan@lina.ai', 'Alex Morgan', 'guest', 'Household Lead');
  };
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 flex items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-violet-500 selection:text-white">
      ...
      <div className="relative z-10 w-full max-w-md rounded-3xl bg-slate-900/80 border border-slate-800/80 backdrop-blur-2xl shadow-2xl shadow-violet-950/50 p-6 sm:p-8 flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-300">
        ...
        {/* Demo / Guest Explore Button */}
        <div className="pt-2 border-t border-slate-800/80 flex flex-col items-center gap-2">
          <button
            type="button"
            onClick={handleGuestDemo}
            className="w-full py-2.5 px-4 rounded-2xl bg-slate-950/60 hover:bg-slate-800/60 border border-slate-800 text-xs font-semibold text-slate-300 hover:text-white flex items-center justify-center gap-2 transition-all"
          >
          ...
          <div className="flex items-center gap-1 text-[10px] text-slate-500">
            ...
            <span>Encrypted local session • Multi-Model AI Ready</span>
          </div>
        </div>
